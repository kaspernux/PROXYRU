	...
	if (!file_exists('kaspernux.install')) {
		if (file_exists('../config.php') && file_exists('../index.php') && file_exists('../texts.json') && file_exists('../sql/sql.php')) {
			...
			if ($getTokenStatus == true) {
				...
				file_put_contents('kaspernux.install', json_encode(['development' => '@Proxy007', 'install_location' => $_POST['install_location'], 'main_domin' => $domain, 'token' => $_POST['token'], 'dev' => $_POST['admin-id'], 'db_name' => $_POST['db-name'], 'db_username' => $_POST['db-user'], 'db_password' => $_POST['db-pass']], 448));
